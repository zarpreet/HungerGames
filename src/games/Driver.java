...
public class Driver {
    ...
    private static void testEliminateDueler(HungerGames studentHungerGames, DuelPair pair) {
        ...
        if (pair.getPerson1() != null && pair.getPerson2() != null) {
            ...
            StdOut.print(winner.getFirstName() + " " + winner.getLastName() + " won against ");
            if (winner == pair.getPerson1()) {
                StdOut.print(pair.getPerson2().getFirstName() + " " + pair.getPerson2().getLastName());
            } else if (winner == pair.getPerson2()) {
                StdOut.print(pair.getPerson1().getFirstName() + " " + pair.getPerson1().getLastName());
            }
            StdOut.println();
        } else {
            StdOut.println("One of the people in this pair is empty!");
        }
        StdOut.println("New Tree:\n");
        printTree(studentHungerGames.getRoot());
    }
